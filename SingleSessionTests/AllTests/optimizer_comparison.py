...
OPTIMIZERS = {
    "Adam (current)": lambda params: torch.optim.Adam(params, lr=0.001, weight_decay=1e-3),
    "AdamW": lambda params: torch.optim.AdamW(params, lr=0.001, weight_decay=1e-3),
    "RMSprop": lambda params: torch.optim.RMSprop(params, lr=0.001, weight_decay=1e-3),
    "SGD+momentum": lambda params: torch.optim.SGD(params, lr=0.01, momentum=0.9, weight_decay=1e-3),
}


def train_one_fold(optimizer_name, X_train, y_train, X_test, y_test, num_classes):
...
def main():
    ...
    for opt_name in OPTIMIZERS:
        ...
        for fold, (train_idx, test_idx) in enumerate(cv.split(X, y), start=1):
            ...
            acc = train_one_fold(opt_name, X_train, y_train, X_test, y_test, num_classes)
            scores.append(acc)
            print(f"Fold {fold}: {acc:.4f}")

        results[opt_name] = (np.mean(scores), np.std(scores))
        print(f"{opt_name}: {np.mean(scores):.4f} ± {np.std(scores):.4f}\n")
